...
func GetTotalClearancePerMonthClaimed(db sqlx.Ext) ([]*TotalClearanceMonthly, error) {

	clearanceData := make([]*TotalClearanceMonthly, 0)
	var ID int64
	var DateCreated string
	rows, err := db.Queryx(`SELECT DISTINCT(ID),DateCreated FROM Clearance WHERE DocumentStatus='Claimed'`)
...
func GetTotalReferralsPerMonthClaimed(db sqlx.Ext) ([]*TotalReferralsMonthly, error) {

	referralsData := make([]*TotalReferralsMonthly, 0)
	var ID int64
	var DateCreated string
	rows, err := db.Queryx(`SELECT DISTINCT(ID),DateCreated FROM Referrals WHERE DocumentStatus='Claimed'`)
...
func GetTotalIndigenciesPerMonthClaimed(db sqlx.Ext) ([]*TotalIndigenciesMonthly, error) {

	indigenciesData := make([]*TotalIndigenciesMonthly, 0)
	var ID int64
	var DateCreated string
	rows, err := db.Queryx(`SELECT DISTINCT(ID),DateCreated FROM Indigencies WHERE DocumentStatus='Claimed'`)
