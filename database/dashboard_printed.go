...
func GetTotalClearancePerMonthPrintted(db sqlx.Ext) ([]*TotalClearanceMonthly, error) {

	clearanceData := make([]*TotalClearanceMonthly, 0)
	var ID int64
	var DateCreated string
	rows, err := db.Queryx(`SELECT DISTINCT(ID),DateCreated FROM Clearance WHERE DocumentStatus='For Printing'`)
...
func GetTotalReferralsPerMonthPrintted(db sqlx.Ext) ([]*TotalReferralsMonthly, error) {

	referralsData := make([]*TotalReferralsMonthly, 0)
	var ID int64
	var DateCreated string
	rows, err := db.Queryx(`SELECT DISTINCT(ID),DateCreated FROM Referrals WHERE DocumentStatus='For Printing'`)
...
func GetTotalIndigenciesPerMonthPrintted(db sqlx.Ext) ([]*TotalIndigenciesMonthly, error) {

	indigenciesData := make([]*TotalIndigenciesMonthly, 0)
	var ID int64
	var DateCreated string
	rows, err := db.Queryx(`SELECT DISTINCT(ID),DateCreated FROM Indigencies WHERE DocumentStatus='For Printing'`)
